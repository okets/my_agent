...
| S5 | Test Cleanup + UX Fixes | Planned | Delete 5 orphaned test files, fix 2 stale tests. Auto-create `.enabled` on build. **Nina's UX feedback:** add `desktop_focus_window` as 8th required tool, include `scaleFactor` in screenshot metadata. All tests green. |
