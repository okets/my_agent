...
| S4 | Template & Agent Verification | Planned | Write `skills/capability-templates/desktop-control.md` template with three-tier tool contract. Build reset script. **S1 deferred:** tool schema validation in test harness, functional screenshot test. **S3 deferred:** fix dead crash monitoring, add enabled-gate to factory registration, expand test fixture to 7 tools. **Build loop:** delete → ask agent → harness passes → iterate template. **Acceptance test:** Nina reads text from open Kwrite document. **User feedback:** Nina reflects on tool UX (which tools used, what's missing, coordinate scaling, optional tools). Insights fed back into template. |
