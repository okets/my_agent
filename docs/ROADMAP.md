...
| S2.5 | Streaming Broadcast | Planned | Move streaming broadcasts (text_delta, done) from WS adapter to App event layer. All callers of `sendMessage()` auto-broadcast to WS clients. Remove `conversation_ready` workaround. Fix 6 broken S1 test mocks. [Plan](../sprints/m9.4-s2.5-streaming-broadcast/plan.md) |
