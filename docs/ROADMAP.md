...
| **M9.5: Capability Framework v2** | **Re-opened** | 6 sprints done (S1-S6). S7 planned: browser control as multi-instance capability. |
...
NEXT (M9.5-S7)
══════════════
M9.5-S7 Browser Capability — migrate @playwright/mcp into first multi-instance capability type (one folder per browser, per-instance user-data-dir), generalize settings UI for multi-instance, agent-builds-from-scratch acceptance gate
...
| S7 | Browser Capability | Planned | Migrate `@playwright/mcp` from hardcoded registration into `browser-control` capability. First multi-instance capability type — each browser (Chrome, Edge, Firefox, …) is its own folder with its own `user-data-dir`. Settings UI generalized to render per-instance toggles + "Add another" affordance. Nina routes via standing orders, not a preference. Acceptance gate: agent builds browser capability from scratch against the template. [Plan](../sprints/m9.5-s7-browser-capability/plan.md) |
