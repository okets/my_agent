...
| S4 | Template & Agent Verification | Planned | Write `skills/capability-templates/desktop-control.md` template with three-tier tool contract. Build reset script for clean-state testing. **S1 deferred items:** tool schema validation against template contract in test harness, functional screenshot test (`desktop_screenshot` call + PNG validation). **S3 deferred items:** fix dead crash monitoring code in app.ts (remove or implement via SDK lifecycle), add enabled-gate to factory registration (`registry.get()` instead of `registry.list().find()`), expand test fixture to all 7 required tools. Delete desktop capability → ask agent to build it from scratch → test harness must pass → tools must work. Iterate until agent reliably single-shots it. |
