...
| S11 | Template smoke fixtures | Planned | Every capability template (audio-to-text, text-to-audio, text-to-image, browser-control, desktop-control) ships a `scripts/smoke.sh` contract + `fallback_action` frontmatter. [§12.3](sprints/m9.6-capability-resilience/plan-universal-coverage.md#123-sprint-11--template-smoke-fixtures) |
| S12 | PostToolUseFailure CFR hook + automation-origin wiring | Planned | MCP-plug detection via SDK's typed error hook. Registry `findByName`. Brain + automation-executor both attach detector. Ack-delivery gains automation/system branches; `CFR_RECOVERY.md` writer; debrief-prep reader extension. Orchestrator mutex attach-origin + terminal drain per §4.7. [§12.4](sprints/m9.6-capability-resilience/plan-universal-coverage.md#124-sprint-12--posttoolusefailure-cfr-hook--automation-origin-wiring) |
