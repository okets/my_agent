...
| **M9.6: Capability Resilience & Recovery** | **In Progress** | Phase 1 (S1-S8) + **Phase 2 closed at S15** (S9-S15 done) + **Phase 3 S16-S20 done** (fix-engine swap; reflect collapse + Item B; duplicate TTS path + 4 §0.5 deferrals; UX polish; S20 test infra + terse contract + FU-8 + test triage). **S20 live /pair-browse on 2026-04-20 exposed 4 production wiring bugs** (`s20-test-report.md`): AckDelivery not wired to TransportManager, brain races CFR before fix completes, reprocessTurn never called for STT, terse SKILL.md not synced to instance copy. Plus `cfr-exit-gate-conversation.test.ts` failed (timeout, likely related). **1 fix sprint added: S21 — milestone-close production wiring fixes.** M9.6 does not close until S21 closes. M10 architecturally unblocked at Phase 2 close; per CTO 2026-04-17, M10 work waits until M9.6 closes at S21. |
...
| S21 | M9.6 milestone-close fix sprint — production wiring bugs from S20 live test + brain-awareness gap from S21 retest | **In Progress** | BUGs 1-5 fixed in initial S21 work; live retest 2026-04-21 confirmed those fixes but exposed BUG-6. (1) Wire `AckDelivery` with live `TransportManager` in `app.ts` — ✅ fixed. (2) Brain-CFR race gate (input) — ✅ fixed. (3) Trace + fix `reverifyAudioToText → reprocessTurn` chain — ✅ fixed. (4) Sync terse SKILL.md to `.my_agent/brain/skills/` + automated sync — ✅ fixed. (5) `cfr-exit-gate-automation` precondition — ✅ fixed. **(6) NEW — added 2026-04-21:** brain unaware of degraded output capabilities; falsely claims voice works in text-fallback replies. Fix: degraded-capability section in system prompt, sourced from registry; no per-capability code; brain authors acknowledgement in own voice. Generic — works for any future capability degradation. (7) Live /pair-browse retest after BUG-6 — full CTO sign-off. **M9.6 closes here. M10 unblocks.** [§2.6](sprints/m9.6-capability-resilience/plan-phase3-refinements.md#26-sprint-21--m96-milestone-close-fix-sprint) · [Live retest](sprints/m9.6-capability-resilience/s21-live-retest.md) |
