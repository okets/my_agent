...
| **M9.4: Conversation UX/UI** | **Done (+ S4.1 supplemental merged)** | 7 total — 6 original + S4.1 (brief section preservation + delivery-ack correctness, merged 2026-04-20). S5 closed UX-1 (handoff continuity); S6 closed UX-2 (progress cadence); S4.1 closed the 2026-04-20 morning-brief silent-drop incident plus 2 downstream delivery-ack hazards. |
...
| **M9.6: Capability Resilience & Recovery** | **In Progress** | 23 sprints done (S1-S23 — S22 awaiting CTO sign-off, S23 in final stages per CTO 2026-04-21). Reactive recovery surface complete: input/output/tool capabilities × Modes 1+2+3. **S24 added** per CTO direction *"any plug-side failure should be recoverable"* + *"the user should not be bothered with internal fixes if done in background"* — Mode 4: daily proactive probe via heartbeat, silent self-heal via system-origin CFR, surfaces in next morning's brief. Audit (2026-04-21) found 5 issues folded into the plan: missing `systemOrigin()` factory (use inline literal), private `inFlight` (add public `isInFlight`), system-origin terminal drain doesn't write outcome to ring buffer (latent S19 gap, fixed in S24), `testAll()` MCP coverage needs sprint-time verification, brief composer is `debrief-reporter` (need `AckDelivery` constructor dep). After S23 + S24, framework matrix is complete: 3 capability shapes × 4 MCP failure modes. M10 still blocked. |
...
M9.6 CLOSED 2026-04-21 — M10 UNBLOCKED
════════════════════════════════════════
...
### M9.4: Conversation UX/UI — DONE (+ S4.1 supplemental merged 2026-04-20)
