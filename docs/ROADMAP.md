...
| **M9.4: Conversation UX/UI** | **Done (+ S4.1 merged; S4.2 in soak with fu1/fu2/fu3; S4.3 planned)** | 9 total — 6 original + S4.1 (brief section preservation + delivery-ack correctness, merged 2026-04-20) + **S4.2 (proactive delivery as action request — code complete 2026-04-27; soak Day-1/Day-2/Day-3 surfaced cascading framework bugs addressed in fu1/fu2/fu3)** + **S4.3 (worker pipeline cleanup — single writer + sidecar metadata; planned)**. S4.1 closed the 2026-04-20 silent-drop incident; S4.2 addresses the 2026-04-25–27 dismissal pattern (delivery-time framing); S4.2 soak surfaced a framework-level worker-pipeline bug (executor overwriting worker `deliverable.md` with model response stream, traced to drift commits across Apr 1 / Apr 6 / Apr 10) addressed in fu3; S4.3 closes the deferred forward-improvements (chart augmentation as worker-runtime tool, capability template migration to `result.json` sidecar, sidecar convention codified). |
...
| **M9.6: Capability Resilience & Recovery** | **Done** | 24 sprints (S1-S24). Framework matrix complete: 3 capability shapes (input/output/tool) × 4 MCP failure modes (tool exception, child crash, server startup crash, daily probe) × 3 user-visibility contracts (notified, silent+ack, silent+brief). Architect APPROVE 2026-04-22 (CTO + architect present at live retest). M10 unblocked. |
...
M9.6 CLOSED 2026-04-22 — M10 UNBLOCKED
════════════════════════════════════════
...
### M9.4: Conversation UX/UI — DONE (+ S4.1 merged 2026-04-20; S4.2 in soak with fu1/fu2/fu3; S4.3 planned)
...
| S4.1 | Brief Section Preservation + Delivery-Ack Correctness (supplemental) | **Done** | Fixes two latent silent-drop classes in S4's delivery chain, both exposed 2026-04-20. **(A) Content truncation:** removed byte-slice in `summary-resolver.ts`; added 100K fail-loud stub; `WRAPPER_MARKER` HTML-comment contract between aggregator (handler-registry.ts) and resolver, with silent-break guard test; post-Haiku heading verification + raw-content fallback; strengthened condense prompt preserves retry-attempt wrappers. **(B) Delivery-ACK correctness:** both `alert()` and `initiate()` now observe `sendSystemMessage()` outcome; new `skipped_busy` + `send_failed` statuses; `initiate()` returns `{ conversation, delivery: AlertResult }`; all 6 callers updated (heartbeat, alert channel-switch, automation-processor/scheduler, app.ts notifier [fixed pre-existing dead `if (!alerted)` bug], debug.ts). **(C) Briefing timing:** `markDelivered()` fires after first model output via extracted `ackBriefingOnFirstOutput` helper. Design principles: *truncation may summarize a section, must not drop one; delivery may fail, must not lie.* **Live verification on real server:** 34,271-byte fixture → 5,716-byte Haiku output, 14/14 wrappers preserved. 53/53 sprint-scoped tests pass; full suite 7 failing (was 8 pre-sprint — improvement). External audit PASS-with-amendments, all 5 gaps + FU-6 + FU-7 folded in. [Plan](../sprints/m9.4-s4.1-brief-section-preservation/plan.md) · [Review](../sprints/m9.4-s4.1-brief-section-preservation/review.md) · [Decisions](../sprints/m9.4-s4.1-brief-section-preservation/DECISIONS.md) · [Test report](../sprints/m9.4-s4.1-brief-section-preservation/test-report.md) |
| S4.2 | Proactive Delivery as Action Request (supplemental) | **Code complete (soak surfaced framework bug — see fu1/fu2/fu3)** | Addresses the 2026-04-25–27 dismissal pattern where Nina classifies mid-conversation `[SYSTEM:]` injections as background activity rather than delivery actions. Same fix covers both delivery flavors (`notify: debrief` brief + `notify: immediate` worker output, e.g., daily relocation session). **(A) Trigger conversion:** new `injectActionRequest`/`sendActionRequest` path delivers proactive content as user-role action requests rather than `[SYSTEM:]`-wrapped context. All four `[SYSTEM:]` pre-wrap sites removed; `[Pending Briefing]` becomes `[Pending Deliveries]`; dead `pendingNotifications` queue deleted (zero callers). **(B) Production hygiene:** worker `deliverable_written` validator rejects thinking-trace openings; CFR-fix automations leave brief queue via `manifest.system: boolean`; Haiku preamble stripped; archived 29 disabled automations. **(C) Voice rule restoration:** `## Conversation Voice` restored to `standing-orders.md`; `## Brief Requirements` leftover deleted. **(D) Rollback safety:** `PROACTIVE_DELIVERY_AS_ACTION_REQUEST` env flag; CLAUDE.md drift fixed. **(Soak follow-ups):** fu1 (Day-1: validator regex widened, prompt anti-tomorrow/anti-background framing); fu2 (Day-2: inline deliverable content in prompt body — drop file-reference Read directive that triggered Sonnet tool-call narration); **fu3 (Day-3: worker `deliverable.md` is source of truth — delete `automation-executor.ts:605-621` overwrite block + legacy `deliverable-utils.ts` XML-tag contract; add defense-in-depth validator gate at job-end; resolve cadence-rule contradiction).** Investigation ([`worker-pipeline-{history,mechanism-inventory,redesign}.md`](../sprints/m9.4-s4.2-action-request-delivery/)) traced root cause to Apr-1/Apr-6/Apr-10 drift commits where each fix addressed local pain without owning the worker contract end-to-end. fu3 is the load-bearing framework fix. [Plan](../sprints/m9.4-s4.2-action-request-delivery/plan.md) · [Audit](../sprints/m9.4-s4.2-action-request-delivery/audit.md) · [Soak Day-1](../sprints/m9.4-s4.2-action-request-delivery/soak-day-1.md) · [Soak Day-2](../sprints/m9.4-s4.2-action-request-delivery/soak-day-2.md) · [fu3 plan](../sprints/m9.4-s4.2-action-request-delivery/soak-day-3-followup-plan.md) · [Bug](../bugs/2026-04-27-proactive-delivery-dismissed.md) |
| S4.3 | Worker Pipeline Cleanup — Single Writer + Sidecar Metadata (supplemental, planned) | **Planned (start ≥ 2026-05-05, after S4.2 7-day soak closes)** | Closes the deferred items from S4.2-fu3's redesign. **(A) Chart augmentation as worker-runtime tool:** today the executor's post-run chart code at `automation-executor.ts:649-672` is the third independent writer of `deliverable.md` — appends `![chart](url)` after worker writes. Post-fu3 it's enrichment-not-corruption (reads worker's clean file), but it still violates the single-writer principle. Move to a worker-callable MCP tool (`chart_tools.append_to_deliverable`) OR teach workers to embed the chart URL inline when writing `deliverable.md` (workers already use `chart_tools.create_chart` to generate the SVG). One write path. **(B) Capability template migration to `result.json` sidecar:** `capability_build` and `capability_modify` templates instruct workers to write YAML frontmatter into `deliverable.md`; this conflates user-facing content with framework telemetry. Migrate to `result.json` sidecar — `deliverable.md` is plain markdown for the user, structured metadata (`change_type`, `test_result`, `summary`) lives in `result.json`. Rewrite `completion_report` / `test_executed` / `change_type_set` validators to read the sidecar. **Note (corrected 2026-05-01):** `cfr-fix-*` capability_modify automations are dynamically spawned at runtime by `recovery-orchestrator.ts:412` + `app.ts:758` on capability failures (not from static manifests in `_archive/`). The post-fu3 worker pipeline handles them correctly today — the four frontmatter-aware validators are still wired and parsing YAML; this migration is a code-cleanliness improvement, **not** a correctness fix. See [`post-fu3-capability-implications.md`](../sprints/m9.4-s4.2-action-request-delivery/post-fu3-capability-implications.md). **(C) Codify the sidecar metadata convention:** add to `packages/dashboard/CLAUDE.md` and the framework docs that structured worker metadata uses `result.json` sidecar (JSON, typed) rather than markdown frontmatter. The project's existing "normalized metadata standard" (markdown frontmatter for markdown files) still applies; sidecar is for structured-data-next-to-markdown which is a different shape. **(D) End-to-end test for capability_modify post-fu3:** today no e2e test exercises the full capability spawn → frontmatter deliverable → validators → paper-trail → CFR_RECOVERY.md chain. Add one as part of the migration (verifies both the old and new code paths during transition, and locks the contract long-term). Forward-improvement scope; not blocking morning briefs. Sequencing: starts after S4.2's 7-day soak closes (target 2026-05-05). |
...
- Brief delivery: workers summarize their own work (one summarization), reporter assembles (no LLM), Conversation Nina presents. Three-layer fallback: disk artifact → short stream as-is → Haiku summarize. No truncation at any level. **S4.1 addendum (2026-04-20):** "no truncation" wasn't fully true — the Haiku fallback had a latent 20K byte-slice on its *input* that dropped sections silently on oversize multi-section aggregates. S4.1 removes the slice and codifies the principle *truncation may summarize, must not drop sections*.
...
| S15 | Phase 2 exit gate — E2E incident replay for all installed plug types | Done | TTS CFR detection wired (`synthesizeAudio` → `CapabilityInvoker`). `reverifyTextToAudio` arg contract + MP3 header fix (pre-existing S13 bugs). 4 E2E exit gates green: STT real-incident replay (v2 origin, Songkran reprocess), TTS terminal-path (terminal-fixed ack), browser-chrome synthetic (automation-origin, CFR_RECOVERY.md), desktop-x11 synthetic. S7 regression gate passed. 214/214 dashboard integration + 290 core tests (2 pre-existing skips). MockSessionManager `setTurnContext` stub fixed (S12 follow-through). **Phase 2 closed at S15. M10 architecturally unblocked; per CTO scheduling decision M10 work waits for Phase 3 close.** [Plan](sprints/m9.6-capability-resilience/s15-plan.md) · [Review](sprints/m9.6-capability-resilience/s15-review.md) · [Architect review](sprints/m9.6-capability-resilience/s15-architect-review.md) · [Test report](sprints/m9.6-capability-resilience/s15-test-report.md) |
...
| S16 | Fix-engine swap + wall-time measurement | Done | Orchestrator now invokes `capability-brainstorming` in MODE: FIX (Opus). `buildFixModeInvocation` + `AutomationSpec.targetPath` + `JOB_TIMEOUT_MS = 15 min` + ESCALATE marker handling (redesign-needed / insufficient-context). New surrender ack kinds + copy. SKILL.md Step 0 gate + neutral-identifier convention. R3 regression assertions catch SKILL.md edit-clobber. D6 documents Option B sibling-skill escape hatch. New `POST /api/debug/cfr/inject` debug endpoint enables wall-time + future CFR regression testing. Wall-time measured: tts-edge-tts 480 s (Branch B), browser-chrome 652 s (Branch C); per-attempt time is Branch A (122 s, 113 s) — multi-attempt accumulation traced to an upstream orchestrator bug, inherited to S17. M1 mitigation (smoke output in MODE: FIX prompt) inherited to S17. Sprint went REJECTED → APPROVED on re-review after dev addressed B1 (sham wall-time replaced with real-plug measurement) + B2 (premature merge documented in DECISIONS D7) + B3 (premature ROADMAP-Done reverted in cf4cb78). 29/29 acceptance tests, 0 tsc errors. [Plan](sprints/m9.6-capability-resilience/s16-plan.md) · [Decisions](sprints/m9.6-capability-resilience/s16-DECISIONS.md) · [Deviations](sprints/m9.6-capability-resilience/s16-DEVIATIONS.md) · [Architect review](sprints/m9.6-capability-resilience/s16-architect-review.md) · [Test report](sprints/m9.6-capability-resilience/s16-test-report.md) · [Wall-time results](sprints/m9.6-capability-resilience/s16-walltime-results.md) |
| S17 | Reflect-phase collapse + S16 inherited items (M1 + orchestrator-iteration investigation) | Done | Reflect-phase removed cleanly: `REFLECTING` / `REFLECT_JOB_DONE` / `SPAWN_REFLECT_JOB` / `reflectJobId` / `renderReflectPrompt` all purged from production code. `MAX_JOBS` 5→4. `fix-automation.md` deleted. **Item B (substantive win):** root cause of S16 wall-time multi-attempt accumulation traced to `KNOWN_TERMINAL` mismatch — `automation-executor.ts` sets `job.status = "completed"` for success but `awaitAutomation` only recognised `"done"`. Every successful CFR fix-mode (and pre-S16 reflect+execute) attempt has been returning `{status:"failed"}` to the orchestrator since Phase 1, causing spurious 2nd+3rd attempts. Adapter fix in `app.ts:659,702` extends `KNOWN_TERMINAL` and normalises `"completed"`→`"done"`. Bug closes silently — no migration needed. **Item A:** `AutomationSpec.smokeOutput?` field + `## Smoke Output` section in MODE: FIX prompt. **FU-1/FU-2:** ESCALATE path now writes paper-trail attempt + warns on unrecognised reasons. 6 commits exactly per architect-amended plan; cleanest Phase 3 sprint to date — full §0.3 compliance, all sprint artifacts substantive. [Plan](sprints/m9.6-capability-resilience/s17-plan.md) · [Decisions](sprints/m9.6-capability-resilience/s17-DECISIONS.md) · [Deviations](sprints/m9.6-capability-resilience/s17-DEVIATIONS.md) · [Architect review](sprints/m9.6-capability-resilience/s17-architect-review.md) · [Test report](sprints/m9.6-capability-resilience/s17-test-report.md) · [Investigation](sprints/m9.6-capability-resilience/proposals/s17-orchestrator-iteration-investigation.md) |
| S18 | Duplicate TTS path collapse + four §0.5 inherited Phase 2 deferrals | Done | `chat-service.synthesizeAudio` is the sole authoritative TTS path. `wireAudioCallbacks` deleted; `BaileysPlugin.onSendVoiceReply` no longer assigned. Transport interface split: `sendAudioUrlViaTransport` reads pre-synthesized files + `sendTextViaTransport` for explicit text fallback. `MessageHandlerDeps` updated; message-handler captures `audioUrl` from every `done` event and follows the §2.3 5-row fallback table (split-done with audioUrl, final-done with audioUrl, empty audioUrl, error-event catch path, tool-only). **Four §0.5 inherited Phase 2 deferrals all landed:** S10-FU-2 / S13-FU-1 (bash wrapper removed from `reverifyAudioToText`; invoker required via runtime guard); S11-FU-2 (text-to-audio template smoke validates OggS magic bytes); S11-FU-5 (`tts-edge-tts` transcodes to real Ogg/Opus via ffmpeg — verified on disk, gitignored per .my_agent/ privacy); S15-FU-4 option (a) (`reverifyTextToAudio` strict Ogg only — MP3/WAV/ID3 paths removed). 14 new tests + tts-paths fallback table coverage + voice-reply regression + cfr-tts-single-emit (`ttsRunCalls.length === 1` — improved beyond spec). 9 commits exactly per architect-amended plan; back-to-back clean Phase 3 sprint with S17. D2: `Reverifier.invoker?` stays optional with target sprint S20 for type tightening. Dev caught a plan-text typo in OggS magic bytes (`4f677353` → `4f676753`). [Plan](sprints/m9.6-capability-resilience/s18-plan.md) · [Decisions](sprints/m9.6-capability-resilience/s18-DECISIONS.md) · [Deviations](sprints/m9.6-capability-resilience/s18-DEVIATIONS.md) · [Architect review](sprints/m9.6-capability-resilience/s18-architect-review.md) · [Test report](sprints/m9.6-capability-resilience/s18-test-report.md) |
| S19 | UX polish — ack coalescing + notifier fan-out + system-origin UI + frontmatter friendly_name + assistant-turn orphan + failure_type producer | Done | Six features end-to-end. **Ack coalescing:** 30s window per conversation, N-aware Oxford comma, partial restoration ("X is back; Y still in progress"), combined terminal copy (fixed-only / surrendered-only / mixed), cross-origin bypass. **AutomationNotifierLike** concrete impl wired in app.ts using `conversationInitiator.alert() → initiate()` mediator-framing pattern. **Fixed-outcome bug** (S12 inherited): outcome now derived from `context.kind`, not hardcoded "surrendered". **System-origin ring buffer** (max 100 events) + `/api/capabilities/cfr-system-events` endpoint + Tailwind-styled health panel in Settings. **TranscriptTurn.failure_type** structured field + producer in `chat-service.ts:934` (Task 5.5 added by architect after R1 catch — plan v0 mislocated to message-handler) + watchdog scan with idempotency. **FRIENDLY_NAMES** migrated to per-template `friendly_name:` frontmatter via `registry.getFriendlyName()` with hardcoded table as fallback (5 templates updated; image-to-text deferred per FU-2). **Largest-surface Phase 3 sprint and the cleanest** — third clean §0.3-compliant sprint in a row (S17+S18+S19). External auditor used; verified all 15 spec requirements + 34 new tests pass. R1 producer + R2 ring-buffer test correction + S1-S5 suggestions all addressed. [Plan](sprints/m9.6-capability-resilience/s19-plan.md) · [Decisions](sprints/m9.6-capability-resilience/s19-DECISIONS.md) · [Deviations](sprints/m9.6-capability-resilience/s19-DEVIATIONS.md) · [Architect review](sprints/m9.6-capability-resilience/s19-architect-review.md) · [External review](sprints/m9.6-capability-resilience/s19-review.md) · [Test report](sprints/m9.6-capability-resilience/s19-test-report.md) |
| S20 | Phase 3 exit gate — two definitive smoke tests + test-suite triage | **Partial — exposed 4 production bugs (S21 follows)** | **§2.5.0** Zero-failed-tests gate: 3 inherited failures root-caused (visual baseline regenerated; `handleWsMessage` rename; `CapabilityInvoker` missing from test app). **§2.5.0b** FU-8 closed: `response` → `chunks[]` + inline `join("")`. **§2.5.1** Terse deliverable contract: SKILL.md Step 5 rewritten + gate test extended + new `fix-mode-deliverable-contract.test.ts` (6 tests). **§2.5.2** Exit-gate E2E infra delivered: `cfr-exit-gate-helpers.ts` + automation/conversation/abbreviated test files. Unit suite 1347/0; abbreviated replays passed (TTS terminal + desktop-x11). **BUT live /pair-browse on 2026-04-20 revealed 4 production bugs:** (1) `AckDelivery` not wired to live `TransportManager` — every "hold on" ack drops; (2) brain races CFR — replies "Voice transcription is down again" 11s after voice arrives, before fix completes; (3) `reprocessTurn` never called for STT — voice message silently dropped even after fix; (4) terse SKILL.md not synced to `.my_agent/` instance copy — production fix agent still writes 3K verbose deliverables. `cfr-exit-gate-conversation.test.ts` also failed (timeout — likely related to bug #3). **§0.3 correction:** dev pre-marked ROADMAP-Done before architect/CTO approval (same regression S16 made; reverted before commit). M9.6 does not close until S21. [§2.5](sprints/m9.6-capability-resilience/plan-phase3-refinements.md#25-sprint-20--phase-3-exit-gate-two-definitive-smoke-tests) · [Decisions](sprints/m9.6-capability-resilience/s20-DECISIONS.md) · [Deviations](sprints/m9.6-capability-resilience/s20-DEVIATIONS.md) · [Follow-ups](sprints/m9.6-capability-resilience/s20-FOLLOW-UPS.md) · [Architect review](sprints/m9.6-capability-resilience/s20-architect-review.md) · [Test report](sprints/m9.6-capability-resilience/s20-test-report.md) |
| S21 | M9.6 milestone-close fix sprint — production wiring bugs from S20 live test + brain-awareness gap from S21 retest | **Done — for input + output capabilities only** | All 6 production bugs fixed for input/output recovery loops. BUG-1: `AckDelivery` wired with live `TransportManager`. BUG-2: brain-CFR race gate (**STT input only**). BUG-3: `reverifyAudioToText → reprocessTurn` chain. BUG-4: terse SKILL.md synced to instance. BUG-5: `cfr-exit-gate-automation` precondition. BUG-6: `VOICE_MODE_HINT` missing from gate-resolved path; system prompt degraded-capability injection added. Live retest 2026-04-21 (STT + TTS): full input/output recovery chain verified end-to-end. CTO verdict: "worked pretty well." **But same retest day, breaking `browser-chrome` exposed S22 (tool-capability gap).** S21 work itself is complete and correct — the gap is architectural scope, not a S21 regression. [§2.6](sprints/m9.6-capability-resilience/plan-phase3-refinements.md#26-sprint-21--m96-milestone-close-fix-sprint) · [Live retest](sprints/m9.6-capability-resilience/s21-live-retest.md) · [Decisions](sprints/m9.6-capability-resilience/s21-DECISIONS.md) · [Review](sprints/m9.6-capability-resilience/s21-review.md) |
| S22 | Tool capability recovery loop — close the third shape of the framework | **Done** | **BUG-7 implementation complete:** `interaction: input \| output \| tool` frontmatter on CAPABILITY.md + scanner + registry; `retryTurn` orchestrator path; 22 new tests across 3 dispatch shapes (tool/input/output); E2E exit-gate test; suite green (1370/0 dashboard, 687/0 core). All design decisions in `s22-DECISIONS.md` D1-D13. **First live retest masked S22's path** (config-corruption broke MCP at boot, brain fell back to Desktop MCP's parallel browser tools — intentional per D11). **Methodology correction (D11-D13)** specified an (a)-shape failure (binary/runtime corruption so MCP starts cleanly + tool call fails) and explicit-capability prompt. **Corrected live retest 1 PASS (2026-04-21):** `browser-chrome/src/server.ts` crash-on-first-`tools/call` injection → `PostToolUseFailure` fired → CFR ack(attempt) → brain notified user "hold on — fixing" → fix agent rewrote server.ts → reverify pass → `terminal-fixed` ack → `retryTurn` fired → CNN screenshot delivered. Full chain end-to-end. **Live retest 2 (voice regression):** PASS, inherited from S21 — S22 doesn't touch input/output paths. **§0.3:** S22 dev did NOT pre-mark ROADMAP-Done (first M9.6 sprint to follow this discipline). **BUG-8 (deferred to M10):** ack-ordering wart observed in retest as expected. **FOLLOW-UP-3 (promoted to S23 in scope, 2026-04-21):** Mode 3 detection unverified for MCP-init failures. S22 dev observed `processSystemInit` log silence during the first retest's (b)-shape failures; per CTO direction *"any plug-side failure should be recoverable,"* this becomes S23's scope rather than M10. [§2.7](sprints/m9.6-capability-resilience/plan-phase3-refinements.md#27-sprint-22--tool-capability-recovery-loop) · [Gap report](sprints/m9.6-capability-resilience/tool-capability-cfr-gap.md) · [Decisions](sprints/m9.6-capability-resilience/s22-DECISIONS.md) · [Live retest](sprints/m9.6-capability-resilience/s22-live-retest.md) · [Architect review](sprints/m9.6-capability-resilience/s22-architect-review.md) · [Review](sprints/m9.6-capability-resilience/s22-review.md) · [Follow-ups](sprints/m9.6-capability-resilience/s22-FOLLOW-UPS.md) |
| S23 | Mode 3 verification + matcher fix — any plug-side failure recoverable | **Done** | S22 wired three capability shapes (input/output/tool) for MCP failure Modes 1+2 (`PostToolUseFailure`-based: tool exception, child crash mid-call). Mode 3 (MCP server crashes at startup, tools never register) has detection wired since S12 (`processSystemInit` in `mcp-cfr-detector.ts:127-165`) but was never verified end-to-end against a real plug. S22 retest's first 3 attempts engineered Mode 3 failures and observed: no `processSystemInit` log, no CFR detection. Two candidate causes: (1) SDK omits failed servers from `mcp_servers[]` for the `system_init_raw` event when stdio crashes immediately; (2) SDK's status string for crash-at-boot doesn't match the matcher's `"failed" \| "needs-auth" \| "disabled"` allow-list. **Diagnostic-driven small sprint:** add debug log capturing actual `mcp_servers[]` shape, fix matcher to accept observed strings, fall back to `q.mcpServerStatus()` health probe if init frame omits failed servers entirely. Live retest: corrupt `browser-chrome` config (b-shape), observe `processSystemInit` → CFR → fix-mode → terminal-fixed → subsequent explicit-capability prompt succeeds. [§2.8](sprints/m9.6-capability-resilience/plan-phase3-refinements.md#28-sprint-23--mode-3-verification--matcher-fix) · [Decisions](sprints/m9.6-capability-resilience/s23-DECISIONS.md) · [Deviations](sprints/m9.6-capability-resilience/s23-DEVIATIONS.md) · [Follow-ups](sprints/m9.6-capability-resilience/s23-FOLLOW-UPS.md) · [Architect review](sprints/m9.6-capability-resilience/s23-architect-review.md) · [Test report](sprints/m9.6-capability-resilience/s23-test-report.md) |
| S24 | Mode 4 — daily silent self-heal + brief integration | **Done** | All five planned sub-tasks landed clean (suite: 1391/0 dashboard, 695/0 core; TypeScript clean). (1) Heartbeat callback wired at 24h cadence in `app.ts`, uses inline `{ kind: "system", component: "capability-health-probe" }` (D5 — spec's `orchestrator.emitFailure` was stale; correct entry point is `app.cfr.emitFailure`). (2) `RecoveryOrchestrator.isInFlight(type)` public method at `recovery-orchestrator.ts:137-143`. (3) **Latent S19 ring-buffer gap fixed** — new `AckDelivery.recordSystemOutcome` API + drain calls it; `SystemCfrEvent.outcome` widened to include `"fixed"`; `deliver()` system-branch fixed to transition (not duplicate) entries. (4) D6 documents `testAll()` MCP coverage verification — both manifests already correct, no fix needed; new `test-harness-mcp-coverage.test.ts` (7 tests) locks the "never silent untested" invariant. (5) `formatSystemHealthSection()` in `handler-registry.ts:307-336` with **24h cutoff filter** (D8); `BuiltInHandler` ctx extended with `ackDelivery?: AckDelivery`; plumbed via `automation-executor` + `debrief-automation-adapter`. **In-sprint deviation DEV-1** (system-origin reverify): live retest exposed that `reverifyAudioToText` requires `rawMediaPath`, which system-origin probes don't carry. Fixed: `dispatchReverify` early-returns `{pass: true}` after `waitForAvailability()` for `origin.kind === "system"` — rescan + testAll IS the verification. Live retest 2026-04-22 (CTO + architect present): probe fires, capability marked degraded, CFR system-origin emitted, **silent path verified** (chat panel completely untouched), fix automation spawned + repaired transcribe.sh, ring buffer transitioned to `fixed`, dashboard clean. All 6 §2.9.6 gates PASS. Architect APPROVE (unconditional). Framework matrix complete. [§2.9](sprints/m9.6-capability-resilience/plan-phase3-refinements.md#29-sprint-24--mode-4-daily-silent-self-heal--brief-integration) · [Decisions](sprints/m9.6-capability-resilience/s24-DECISIONS.md) · [Deviations](sprints/m9.6-capability-resilience/s24-DEVIATIONS.md) · [Follow-ups](sprints/m9.6-capability-resilience/s24-FOLLOW-UPS.md) · [Test report](sprints/m9.6-capability-resilience/s24-test-report.md) · [Architect review](sprints/m9.6-capability-resilience/s24-architect-review.md) · [External review](sprints/m9.6-capability-resilience/s24-review.md) |
