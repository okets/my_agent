...
| **M9.6: Capability Resilience & Recovery** | **In Progress** | 21 sprints done. S21 closed cleanly for input (STT) and output (TTS) capabilities — all 6 wiring bugs fixed and verified in live retest 2026-04-21. **But the same retest day, breaking `browser-chrome` exposed a structural gap** (`tool-capability-cfr-gap.md`): tool capabilities (browser-control, desktop-control) have no recovery loop — failure happens mid-brain-session (no gate possible), reverify produces no `recoveredContent`, orchestrator's `terminalDrain` skips replay, original task silently dropped. The framework was implicitly STT-shaped throughout M9.6. **S22 added: tool-capability recovery loop** — adds `interaction:` frontmatter + orchestrator `retryTurn` path alongside the existing `reprocessTurn`. M9.6 closes when tool-capability live retest works end-to-end. M10 still blocked. |
