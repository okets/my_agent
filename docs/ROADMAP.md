...
| S21 | M9.6 milestone-close fix sprint — production wiring bugs from S20 live test | **Planned** | Three blocker bugs + one process bug + one test-precondition fix + live retest. (1) Wire `AckDelivery` with live `TransportManager` in `app.ts`. (2) Brain-CFR race gate — design + implement: brain must not reply to a turn while CFR is active for that turn (timeout = orchestrator job timeout). (3) Trace + fix `reverifyAudioToText → reprocessTurn` chain in `recovery-orchestrator.ts`. (4) Sync terse SKILL.md to `.my_agent/brain/skills/` + add an automated sync mechanism so this can't drift again. (5) Investigate why `cfr-exit-gate-automation.test.ts` precondition resolved `canRun = false` despite plug present. (6) Live /pair-browse retest — full CTO sign-off. **M9.6 closes here. M10 unblocks.** [§2.6](sprints/m9.6-capability-resilience/plan-phase3-refinements.md#26-sprint-21--m96-milestone-close-fix-sprint) |
