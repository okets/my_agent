...
| **M9.6: Capability Resilience & Recovery** | **In Progress** | 22 sprints done. S21 closed input (STT) + output (TTS) recovery loops. S22 closed tool-capability recovery (browser-control, desktop-control) via `interaction:` frontmatter + `retryTurn`; live retest 1 + 2 PASS, awaiting CTO sign-off. **But S22 retest exposed Mode 3 detection (MCP server crash-at-boot) is wired but never verified end-to-end** (`s22-FOLLOW-UPS.md` FOLLOW-UP-3). Per CTO 2026-04-21 *"any plug-side failure should be recoverable"* — promoted to **S23: Mode 3 verification + matcher fix.** Diagnostic-driven small sprint: log SDK shape, fix matcher to accept actual failure-status strings, fall back to `q.mcpServerStatus()` if init frame omits failed servers, live retest with (b)-shape break. After S23, framework matrix is fully covered: 3 capability shapes × 3 MCP failure modes. M10 still blocked. |
