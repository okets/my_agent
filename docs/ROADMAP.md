...
| **M9.4: Conversation UX/UI** | **Done (+ S4.1, S4.2, S4.3 all merged; S4.2 soak Day 5/7 in flight)** | 9 total — 6 original + S4.1 (merged 2026-04-20) + **S4.2 (proactive delivery as action request — Day 5 of 7-day soak clean; fu1/fu2/fu3 stack stable)** + **S4.3 (worker pipeline cleanup — merged 2026-05-02 at `5366c18`, all 7 items A/B/C/D/E/F/G shipped)**. S4.2 fu3 made worker `deliverable.md` writes authoritative; S4.3 closes the deferred forward-improvements (chart self-service, capability `result.json` sidecar migration, convention codified, e2e test) plus the May-2 incident fixes (heuristic reads on-disk truth, audit metadata in result.json, transcript path in action-request prompt). Master tests: 1477 passed / 24 skipped / 0 failed. M9.4 supplemental thread closed. |
...
### M9.4: Conversation UX/UI — DONE (+ S4.1, S4.2, S4.3 all merged; S4.2 soak Day 5/7)
...
| S4.3 | Worker Pipeline Cleanup — Single Writer + Sidecar Metadata (supplemental) | **Done — merged 2026-05-02 at `5366c18`** | All 7 items shipped. **(A) Chart augmentation as worker self-service:** post-run chart code at `automation-executor.ts:649-672` deleted; workers self-serve via existing `chart_tools.create_chart`. **(B) Capability metadata to `result.json` sidecar:** `capability_build`/`capability_modify` emit `deliverable.md` (plain markdown) + `result.json` (`change_type`, `test_result`, `summary`); validators + paper-trail + `recovery-orchestrator.readDeliverable()` migrated. **(C) Codify sidecar convention** in `packages/dashboard/CLAUDE.md`. **(D) End-to-end test for capability_modify post-fu3** locks the contract (uses real harness + mocked SDK; no fictional API). **(E) Empty-deliverable heuristic fix** at `automation-processor.ts:136`: reads on-disk `result.deliverable` instead of `result.work` (which fu1's anti-narration directive correctly silences); handler-skip clause for handler-based automations. Closes the May-2 incident class (`update-relocation-roadmap` worker false-failed despite clean disk artifact — see [`post-fu3-side-effect-worker-incident.md`](../sprints/m9.4-s4.3-worker-pipeline-cleanup/post-fu3-side-effect-worker-incident.md)). **(F) Audit metadata in `result.json`:** framework writes `audit.transcript_path` + `audit.session_id` post-run (computed via `runDir.replace(/[^a-zA-Z0-9-]/g, '-')` matching SDK encoding; `fs.existsSync` probe makes drift detectable). **(G) Surface transcript path in action-request prompt body:** `formatNotification.job_completed` reads `result.json` and appends `Audit trail: <path>` so the brain has SDK-transcript visibility (CTO audits) without unprompted use. Master tests at merge: 1477 passed / 24 skipped / 0 failed; typecheck clean. Pre-merge audit ([`plan-audit.md`](../sprints/m9.4-s4.3-worker-pipeline-cleanup/plan-audit.md)) found 3 blockers + 4 tightening items; all folded into the merged work. [Plan](../sprints/m9.4-s4.3-worker-pipeline-cleanup/plan.md) · [Audit](../sprints/m9.4-s4.3-worker-pipeline-cleanup/plan-audit.md) · [Side-effect incident report](../sprints/m9.4-s4.3-worker-pipeline-cleanup/post-fu3-side-effect-worker-incident.md) |
