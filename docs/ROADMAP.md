...
| **M9.4: Conversation UX/UI** | **Re-opened — S4.1 supplemental** | 6 done + **S4.1 in progress** (brief section preservation — fixes byte-slice in `summary-resolver.ts` that silently dropped user-facing sections on 2026-04-20). Previously: S5 closed UX-1 (handoff continuity); S6 closed UX-2 (progress cadence). |
...
### M9.4: Conversation UX/UI — RE-OPENED (2026-04-20, supplemental S4.1)
...
| S4.1 | Brief Section Preservation + Delivery-Ack Correctness (supplemental) | **In Progress** | Fixes two latent silent-drop classes in S4's delivery chain, both exposed on 2026-04-20 and validated by external audit (pass-with-amendments). **(A) Content truncation:** `summary-resolver.ts:90` byte-slices at 20K before summarization; replaced with no-slice + strengthened condense prompt + 100K fail-loud stub + post-Haiku heading verification. **(B) Delivery ACK correctness:** `alert()` reports `{status: "delivered"}` even when `sendSystemMessage()` returned early on busy session or yielded `{type: "error"}`; new `skipped_busy` and `send_failed` statuses mirror `transport_failed` retry semantics. **(C) Briefing timing:** `briefingResult.markDelivered()` moves from before-model-invocation (line 799) to after-first-output, so session exceptions don't silently consume briefings. Design principles promoted: *truncation may summarize a section, must not drop one; delivery may fail, must not lie.* Regression fixture: today's real 33 374-byte deliverable. [Plan](../sprints/m9.4-s4.1-brief-section-preservation/plan.md) · **Blocks:** M9.6-S20 |
...
| S20 | Phase 3 exit gate — two definitive smoke tests | **Planned — blocked on M9.4-S4.1** | (1) Working Nina screenshots a website after browser plug deliberately broken. (2) Conversation Nina understands a voice message after STT plug deliberately broken. `AppHarness` + recording mock transport. Abbreviated replays for every other registered plug type. **M9.6 closes here. M10 unblocks.** [§2.5](sprints/m9.6-capability-resilience/plan-phase3-refinements.md#25-sprint-20--phase-3-exit-gate-two-definitive-smoke-tests) · **S20 dev pickup (added 2026-04-20):** CFR-fix automation output contract change — `deliverable.md` becomes a terse one-liner per capability (outcome + file changed, if any; 2–5 lines total across all attempts) because the debrief-reporter aggregator reads `deliverable.md` first. Move forensic detail (diagnosis, decision log, per-attempt state, validation commands) to a sibling file in the same `run_dir` — suggested `attempts.md` or `forensic.md`. Rationale: after M9.4-S4.1 fixes section-drop, CFR-fix output will still dominate the Haiku 10K condense budget because each attempt produces a ~2–3K forensic deliverable and the aggregate crosses the threshold. User-facing brief signal is "voice capability broken, now fixed" — not the three-attempt forensic diary. Full detail stays on disk for audit. Out of M9.4-S4.1 scope because this is an M9.6 automation template change. Reference: M9.4-S4.1 plan §"Dev note for M9.6-S20". |
