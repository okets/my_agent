...
| **M9.6: Capability Resilience & Recovery** | **In Progress** | S1 done (raw-media persistence + CFR detector, 43 tests). S2 done (deps wiring at App boot, 6 tests). S3 done (capability hot-reload + restart gap closure, 26 tests). S4 done (recovery orchestrator, 28 tests). S5 done (orphaned-turn watchdog + abbreviation turn_corrected ingestion, 9 tests). S6–S7 pending. **Blocks M10 and all downstream work.** [plan.md](sprints/m9.6-capability-resilience/plan.md) |
