...
### M9.4: Conversation UX/UI — DONE (+ S4.1 merged 2026-04-20; S4.2 planned 2026-04-27)
...
| S4.2 | Proactive Delivery as Action Request (supplemental) | **Planned** | Addresses the 2026-04-25–27 dismissal pattern where Nina classifies mid-conversation `[SYSTEM:]` injections as background activity rather than delivery actions. Same fix covers both delivery flavors (`notify: debrief` brief + `notify: immediate` worker output, e.g., daily relocation session). **(A) Trigger conversion:** new `injectActionRequest`/`sendActionRequest` path delivers proactive content as user-role action requests rather than `[SYSTEM:]`-wrapped context. `alert()` and `initiate()` route through it; both `[SYSTEM:]` wrap sites in heartbeat-service (`:312–313`) removed; `[Pending Briefing]` becomes `[Pending Deliveries]` with action-request framing; `pendingNotifications` queue gets a `kind` discriminator so only genuine system events get wrapped. **(B) Production hygiene:** worker `deliverable_written` validator rejects thinking-trace openings (doubled-signal heuristic); CFR-fix automations leave brief queue via the existing `manifest.system: boolean` flag (default `notify: none` for system-flagged manifests); Haiku conversational preamble stripped before delivery with telemetry. **(C) Voice rule restoration:** `## Conversation Voice` section restored to `standing-orders.md` (regression from M6.7 identity/operations split); `## Brief Requirements` leftover deleted (importance is encoded by what's scheduled, not by a duplicated rule list). **(D) Rollback safety:** `PROACTIVE_DELIVERY_AS_ACTION_REQUEST` env flag (default on) for config-level rollback; service restart documented for standing-orders cache invalidation; CLAUDE.md drift fixed; **7-day live soak required before sprint close** (S4 and S4.1 both PASS-then-regressed; this sprint refuses to repeat the pattern). External pre-implementation audit (`audit.md`) flagged 3 critical issues + 13 concerns, all folded into the revised plan. [Plan](../sprints/m9.4-s4.2-action-request-delivery/plan.md) · [Audit](../sprints/m9.4-s4.2-action-request-delivery/audit.md) · [Bug](../bugs/2026-04-27-proactive-delivery-dismissed.md) |
