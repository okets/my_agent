...
| **M8: Visual & Desktop Automation** | **Active** | 3/5 sprints done (S1 visual pipeline, S2 desktop control Linux, S3 Playwright), 863 tests |
...
| S3.5 | Centralized Screenshot Storage | Planned | Refactor: single `.my_agent/screenshots/` folder replacing per-context dirs. Ref-based lifecycle (referenced = alive, unreferenced = 7-day expiry). Simplify producers (no context needed), ref management on message write + context deletion, cleanup job. [spec](../superpowers/specs/2026-03-31-centralized-screenshot-storage-design.md) · [plan](../sprints/m8-s3.5-centralized-screenshots/plan.md) |
