...
| **M9.4: Conversation UX/UI** | **Done (+ S4.1 merged; S4.2 in soak Day 4/7 with fu1/fu2/fu3; S4.3 in branch dev parallel)** | 9 total — 6 original + S4.1 (merged 2026-04-20) + **S4.2 (proactive delivery as action request — Day 4 of 7-day soak observed clean for both delivery types after fu3)** + **S4.3 (worker pipeline cleanup — branch dev started 2026-05-01, merge gated on tests + Day-5+ soak clean, earliest 2026-05-03)**. S4.2 fu3 made worker `deliverable.md` writes authoritative (delete executor overwrite traced to drift commits Apr 1/6/10); S4.3 closes the deferred forward-improvements (chart move, capability `result.json` sidecar migration, convention codified, e2e test). Branch development of S4.3 happens in parallel to S4.2 soak — production soak signal is orthogonal (different worker types) so the two can run simultaneously without contaminating the gate. |
...
### M9.4: Conversation UX/UI — DONE (+ S4.1 merged 2026-04-20; S4.2 in soak Day 4/7; S4.3 in branch dev parallel)
...
| S4.3 | Worker Pipeline Cleanup — Single Writer + Sidecar Metadata (supplemental) | **Planned, branch dev parallel to S4.2 soak** — start 2026-05-01; merge gated on tests + Day-5+ soak clean. Earliest merge 2026-05-03; latest 2026-05-04. ~1 day of work on a branch. | Closes the deferred items from S4.2-fu3's redesign. **(A) Chart augmentation as worker self-service:** delete the post-run chart code at `automation-executor.ts:649-672` (the third independent writer of `deliverable.md`). Workers self-serve via existing `chart_tools.create_chart` and embed the URL inline when writing their deliverable. **(B) Capability metadata migration to `result.json` sidecar:** `capability_build`/`capability_modify` workers emit two files — `deliverable.md` (plain markdown for the user) AND `result.json` (typed framework metadata: `change_type`, `test_result`, `summary`). Validators (`completion_report`, `test_executed`, `change_type_set`), paper-trail writer, and `recovery-orchestrator.readDeliverable()` migrate to read from the sidecar. **Note (per [`post-fu3-capability-implications.md`](../sprints/m9.4-s4.2-action-request-delivery/post-fu3-capability-implications.md)):** `cfr-fix-*` automations spawn dynamically at runtime via `recovery-orchestrator.ts:412` + `app.ts:758` — the existing pipeline handles them correctly today. This migration is code-cleanliness, **not** a correctness fix. **(C) Codify sidecar convention** in `packages/dashboard/CLAUDE.md`: markdown is for humans, JSON is for the framework. Frontmatter-in-markdown remains the standard for static files (CAPABILITY.md, notebook references); sidecar JSON is for runtime worker telemetry. **(D) End-to-end test for `capability_modify` post-fu3** locks the new contract — was the missing piece flagged by the audit. **Testing strategy:** branch develops in parallel with S4.2 soak on master. Three layers: (1) full unit + integration tests on branch, (2) new e2e test from item D, (3) optional dev-mode dashboard at port 4322 with `soak-probe.sh DASHBOARD=...` for live smoke. Production soak signal is orthogonal — S4.2 covers generic/research workers (morning brief + relocation); S4.3 covers capability workers (only fire on capability failures). [Plan](../sprints/m9.4-s4.3-worker-pipeline-cleanup/plan.md) |
