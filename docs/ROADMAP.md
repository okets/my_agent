...
| **M9.4: Conversation UX/UI** | **Done (+ S4.1 merged; S4.2 code complete, soak in progress)** | 8 total — 6 original + S4.1 (brief section preservation + delivery-ack correctness, merged 2026-04-20) + **S4.2 (proactive delivery as action request — code complete 2026-04-27, 7-day soak begins on merge)**. S5 closed UX-1 (handoff continuity); S6 closed UX-2 (progress cadence); S4.1 closed the 2026-04-20 silent-drop incident; S4.2 addresses the 2026-04-25–27 dismissal pattern where mid-conversation `[SYSTEM:]` injections get classified as background context rather than delivery actions. Same fix covers both the morning brief (`notify: debrief`) and `notify: immediate` worker deliveries (e.g., daily relocation session). |
...
### M9.4: Conversation UX/UI — DONE (+ S4.1 merged 2026-04-20; S4.2 code complete 2026-04-27, soak in progress)
...
| S4.2 | Proactive Delivery as Action Request (supplemental) | **Code complete (soak in progress)** | Addresses the 2026-04-25–27 dismissal pattern where Nina classifies mid-conversation `[SYSTEM:]` injections as background activity rather than delivery actions. Same fix covers both delivery flavors (`notify: debrief` brief + `notify: immediate` worker output, e.g., daily relocation session). **(A) Trigger conversion:** new `injectActionRequest`/`sendActionRequest` path delivers proactive content as user-role action requests rather than `[SYSTEM:]`-wrapped context. `alert()` and `initiate()` route through it; all four `[SYSTEM:]` pre-wrap sites (`conversation-initiator.ts:184/268`, `heartbeat-service.ts:313`, `automation-processor.ts:306`, `app.ts:726`) removed; `[Pending Briefing]` becomes `[Pending Deliveries]` with action-request framing; dead `pendingNotifications` queue **deleted** (zero callers verified — eliminates the audit's Top-1 concern entirely rather than fixing it via a discriminator). **(B) Production hygiene:** worker `deliverable_written` validator rejects thinking-trace openings (doubled-signal heuristic); CFR-fix automations leave brief queue via the existing `manifest.system: boolean` flag (default `notify: none` for system-flagged manifests); Haiku conversational preamble stripped before delivery with telemetry; 26 disabled `cfr-fix-*` and 3 `build-*-capability` automations archived to `_archive/`. **(C) Voice rule restoration:** `## Conversation Voice` section restored to `standing-orders.md` (regression from M6.7 identity/operations split); `## Brief Requirements` leftover deleted (importance is encoded by what's scheduled, not by a duplicated rule list). **(D) Rollback safety:** `PROACTIVE_DELIVERY_AS_ACTION_REQUEST` env flag (default on) for config-level rollback; service restart documented for standing-orders cache invalidation; CLAUDE.md drift fixed; **7-day live soak required before sprint close** (S4 and S4.1 both PASS-then-regressed; this sprint refuses to repeat the pattern). External pre-implementation audit (`audit.md`) flagged 3 critical issues + 13 concerns; dead-code audit (`dead-code-audit.md`) flagged 5 deletions; all folded into the revised plan v3. [Plan](../sprints/m9.4-s4.2-action-request-delivery/plan.md) · [Audit](../sprints/m9.4-s4.2-action-request-delivery/audit.md) · [Dead-code audit](../sprints/m9.4-s4.2-action-request-delivery/dead-code-audit.md) · [Decisions](../sprints/m9.4-s4.2-action-request-delivery/DECISIONS.md) · [Bug](../bugs/2026-04-27-proactive-delivery-dismissed.md) |
