...
| S24 | Mode 4 — daily silent self-heal + brief integration | **Planned** | Reactive recovery (S22 + S23) catches failures the user or Nina encounter. S24 closes the proactive surface: a capability that's degraded silently (API key revoked, dependency rot, config drift) auto-heals in the background without bothering the user. **Wire heartbeat's existing `capabilityHealthCheck` hook at 24h cadence** — runs `registry.testAll()`, emits system-origin CFR for any degraded/failed capability not already in flight. Contract: silent (no chat ack, no WhatsApp ping, no brain turn) → fix-mode runs → ring buffer logs outcome → next morning's brief includes a "## System Health" section. Cost: ~$0.0004/day. **Five sub-tasks** (per audit findings): (1) heartbeat callback wire-up + 24h cadence; (2) public `RecoveryOrchestrator.isInFlight(type)` method; (3) **fix latent S19 gap** — system-origin terminal drain doesn't write outcome to ring buffer (without this, brief shows "in progress" forever); (4) verify `testAll()` MCP coverage for browser-chrome / desktop-x11 (frontmatter + dispatch); (5) `AckDelivery` constructor dep on `debrief-reporter` for brief-section read. Live retest: corrupt `transcribe.sh`, accelerate cadence, observe silent recovery + ring-buffer transition + brief section + ZERO user-visible signal. **M9.6 closes here. M10 unblocks.** [§2.9](sprints/m9.6-capability-resilience/plan-phase3-refinements.md#29-sprint-24--mode-4-daily-silent-self-heal--brief-integration) |
