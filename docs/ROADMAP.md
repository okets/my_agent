...
| **M9.6: Capability Resilience & Recovery** | **In Progress** | Phase 1 (S1-S8) + **Phase 2 closed at S15** (S9-S15 done) + **Phase 3 S16 done** (fix-engine swap; wall-time gate Branch B/C with M1+orchestrator-iteration findings inherited to S17). 4 Phase 3 sprints remaining (S17-S20). M10 architecturally unblocked at Phase 2 close; per CTO 2026-04-17, M10 work waits until M9.6 closes at S20. |
...
| S17 | Reflect-phase collapse + S16 inherited items (M1 + orchestrator-iteration investigation) | Planned | Delete REFLECTING state; fix-mode (S16) made it dead code. `MAX_JOBS` 5→4. Remove `reflectJobId` from `FixSession`. Delete `fix-automation.md`. Must ship after S16 green for one sprint. **Inherited from S16:** M1 mitigation (smoke output in MODE: FIX prompt — saves 60–90 s per attempt) + HIGH-priority investigation of orchestrator's 3-attempt iteration when `executeResult.status === "failed"` despite `deliverable.test_result === "pass"`. [§2.2](sprints/m9.6-capability-resilience/plan-phase3-refinements.md#22-sprint-17--reflect-phase-collapse-dead-code-cleanup) · [§2.2.1 inherited](sprints/m9.6-capability-resilience/plan-phase3-refinements.md#221-inherited-from-s16-architect-review-2026-04-19) |
