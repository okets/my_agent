...
| **M9.6: Capability Resilience & Recovery** | **Re-Opened** | Phase 1 (S1-S8) shipped STT-only CFR — handoff at [HANDOFF-cfr-coverage-gap.md](sprints/m9.6-capability-resilience/HANDOFF-cfr-coverage-gap.md) flagged the class failure. Phase 2 ([plan-universal-coverage.md](sprints/m9.6-capability-resilience/plan-universal-coverage.md) v2.2, two red-team passes) adds S9-S18: TriggeringOrigin types + CapabilityInvoker + exec-bit + smoke.sh contract + PostToolUseFailure hook + origin widening (conversation/automation/system) + reflect-collapse + reverify dispatcher + ack coalescing + capability-brainstorming fix-mode + TTS path collapse + two definitive exit-gate smoke tests. |
...
| S9 | Heartbeat Liveness Fix | Planned (late addendum, 2026-04-18; red-team hardened) | Eliminate `job_interrupted` false positives. Heartbeat's only liveness signal was `todos.json:last_activity`, touched solely by 4 todo MCP tools — workers doing legit `WebFetch`/browser/`Bash` work for >5min got falsely flagged. Four layers (defense-in-depth): (1) audit-log liveness fallback via `sdk_session_id`, (2) run-dir recursive mtime as tertiary signal (catches subagent-delegation gaps), (3) 60s minimum-age gate + recheck-before-deliver in `deliverPendingNotifications`, (4) per-automation `health.stale_threshold_ms` manifest override. Trigger incident: 2026-04-18 `thailand-news-worker` job-9d6ba79b alarmed at 00:06 UTC, completed normally at 00:10 UTC, user got false alert. [Plan](../sprints/m9.1-s9-heartbeat-liveness-fix/plan.md) |
...
| S13 | Reflect-phase collapse | Planned | Eliminate REFLECTING state; one job per attempt; budget cap 4 (safety ceiling). Two-commit sequence: state + types → orchestrator behavior. [§12.5](sprints/m9.6-capability-resilience/plan-universal-coverage.md#125-sprint-13--reflect-phase-collapse) |
| S14 | Reverify dispatcher + terminal-on-fix | Planned | Per-type reverifiers (TTS, image-to-text, text-to-image) + `runSmokeFixture` default (out-of-session subprocess). New `RESTORED_TERMINAL` state for plugs without retriable input. Origin-aware terminal routing. [§12.6](sprints/m9.6-capability-resilience/plan-universal-coverage.md#126-sprint-14--reverify-dispatcher--terminal-on-fix) |
| S15 | Ack coalescing + orphan-watchdog extension | Planned | Friendly-name + multi-instance disambiguation + N-aware conversation-origin coalescing (30s window). Assistant-turn orphan via structured `TranscriptTurn.failure_type` field. [§12.7](sprints/m9.6-capability-resilience/plan-universal-coverage.md#127-sprint-15--ack-coalescing--orphan-watchdog-extension) |
| S16 | Fix-engine swap (capability-brainstorming fix-mode) | Planned | Orchestrator invokes Nina's own authoring skill in a hard-gated fix-mode; skill reads plug folder + DECISIONS.md; `writePaperTrail` appends. `targetPath` plumbing (AutomationSpec + dashboard closure). `.my_agent/` write-guard exemption. JOB_TIMEOUT → 15min. [§12.8](sprints/m9.6-capability-resilience/plan-universal-coverage.md#128-sprint-16--fix-engine-swap) |
| S17 | Duplicate TTS path collapse | Planned | `chat-service.synthesizeAudio` becomes authoritative. Per-path fallback table covers error/split/empty cases. Delete Baileys `onSendVoiceReply` synthesis. [§12.9](sprints/m9.6-capability-resilience/plan-universal-coverage.md#129-sprint-17--duplicate-tts-path-collapse) |
| S18 | Milestone exit gate — two definitive smoke tests | Planned | (1) Working Nina screenshots a website after browser plug deliberately broken. (2) Conversation Nina understands a voice message after STT plug deliberately broken. `AppHarness` + recording mock transport. Abbreviated replays for every other registered plug type. [§12.10](sprints/m9.6-capability-resilience/plan-universal-coverage.md#1210-sprint-18--milestone-exit-gate) |
