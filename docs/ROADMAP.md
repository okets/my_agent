...
| S22 | Tool capability recovery loop — close the third shape of the framework | **Implementation done — live test 1 retest pending after methodology correction** | **BUG-7 implementation complete:** `interaction: input \| output \| tool` frontmatter on CAPABILITY.md + scanner + registry; `retryTurn` orchestrator path; 22 new tests across 3 dispatch shapes (tool/input/output); E2E exit-gate test; suite green (1370/0 dashboard, 687/0 core). All design decisions captured in `s22-DECISIONS.md` D1-D10. **First live retest (2026-04-21) failed to exercise S22's path:** dev corrupted `browser-chrome` config 3 different ways — each was an MCP-init failure (server exits at boot), so tools never registered; brain transparently used Desktop MCP's parallel browser tools (intentional per CTO clarification, D11) and the screenshot succeeded without any CFR firing. **Architect verdict (D11-D13):** S22 implementation is correct; live test methodology was wrong. Required fix: corrupt the browser BINARY/runtime path (so MCP starts cleanly but tool calls fail = (a)-shape), and use an explicit prompt ("Use the chrome browser capability to..."). MCP-init failures (b-shape) require Mode 3 detection — separately tracked as FOLLOW-UP-3 (S23/M10 scope). **BUG-8 (deferred to M10):** ack ordering — SDK doesn't expose pre-delivery hook today. M9.6 closes when corrected live retest 1 passes + voice regression gate (live retest 2) passes. [§2.7](sprints/m9.6-capability-resilience/plan-phase3-refinements.md#27-sprint-22--tool-capability-recovery-loop) · [Gap report](sprints/m9.6-capability-resilience/tool-capability-cfr-gap.md) · [Decisions](sprints/m9.6-capability-resilience/s22-DECISIONS.md) · [Review](sprints/m9.6-capability-resilience/s22-review.md) · [Follow-ups](sprints/m9.6-capability-resilience/s22-FOLLOW-UPS.md) |
