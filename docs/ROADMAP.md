...
| S6 | System Automations + Calendar | Planned | Convert work-loop patterns to automation manifests (system vs user automations), remove WorkLoopScheduler, calendar view becomes timeline interface (past=jobs, future=projected runs), debrief becomes user automation (schedulable, editable), memory maintenance becomes system automation (hidden, not editable) |
| S7 | E2E Verification | Planned | Live E2E tests for all 4 trigger types (schedule, watch, channel, manual) + HITL resume flow, final M7 milestone verification |
