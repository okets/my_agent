...
| S7 | Framework/Instance Split | Planned | Move agentic behavior (delegation, routing, memory tools, ops rules) from `.my_agent/` to `skills/`. Instance keeps identity/personality. Fixes stale tool names, enables framework updates to propagate. [Plan](plans/2026-04-07-m9.2-s7-framework-instance-split.md) |
| S8 | Delegation Verification | Planned | Rerun failed S6 delegation tests after S7 fix. Verify brain delegates with correct `create_automation` + todos. [Plan](plans/2026-04-07-m9.2-s8-delegation-verification.md) |
