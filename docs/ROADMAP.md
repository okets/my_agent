...
| **M9.6: Capability Resilience & Recovery** | **In Progress** | Phase 1 (S1-S8) + **Phase 2 closed at S15** (S9-S15 done) + **Phase 3 S16-S19 done** (fix-engine swap; reflect collapse + Item B latent-Phase-1 bug; duplicate TTS path + 4 §0.5 deferrals; UX polish — ack coalescing + notifier fan-out + system-origin UI + frontmatter friendly_name + assistant-turn orphan + failure_type producer). **1 Phase 3 sprint remaining: S20 exit gate — unblocked (M9.4-S4.1 merged 2026-04-20); S20 also picks up CFR-fix worker output contract change (terse `deliverable.md` + forensic sibling file).** M10 architecturally unblocked at Phase 2 close; per CTO 2026-04-17, M10 work waits until M9.6 closes at S20. |
...
| S20 | Phase 3 exit gate — two definitive smoke tests + test-suite triage | **Planned — unblocked (M9.4-S4.1 merged)** | (1) Working Nina screenshots a website after browser plug deliberately broken. (2) Conversation Nina understands a voice message after STT plug deliberately broken. `AppHarness` + recording mock transport. Abbreviated replays for every other registered plug type. **M9.6 closes here. M10 unblocks.** [§2.5](sprints/m9.6-capability-resilience/plan-phase3-refinements.md#25-sprint-20--phase-3-exit-gate-two-definitive-smoke-tests) · **S20 dev pickup (M9.4-S4.1 FU-2, added 2026-04-20):** CFR-fix automation output contract change — `deliverable.md` becomes a terse one-liner per capability (outcome + file changed, if any; 2–5 lines total across all attempts) because the debrief-reporter aggregator reads `deliverable.md` first. Move forensic detail (diagnosis, decision log, per-attempt state, validation commands) to a sibling file in the same `run_dir` — suggested `attempts.md` or `forensic.md`. Rationale: S4.1 fixed section-drop on the brief side; CFR-fix output still dominates the Haiku 10K condense budget because each attempt produces a ~2–3K forensic deliverable. User-facing brief signal is "voice capability broken, now fixed" — not the three-attempt forensic diary. Full detail stays on disk for audit. Reference: M9.4-S4.1 plan §"Dev note for M9.6-S20" and FOLLOW-UPS.md FU-2. · **Test-suite triage gate (Task §2.5.0, added 2026-04-20):** S20 starts by clearing the 3 inherited dashboard test failures (capabilities-singleton-visual baseline mismatch, capability-ack-render `handleWebSocketMessage` removal, whatsapp-before-browser CFR signal drift). Each failure root-caused, not silenced. **Milestone-exit hard gate:** zero failed tests across `packages/dashboard` + `packages/core` at sprint close — the clean test suite is part of the M9.6 exit criteria, not a soft target. · **WRAPPER_MARKER preservation (M9.4-S4.1 handoff §1):** any S20 code path touching debrief aggregation must preserve `handler-registry.ts`'s `<!-- wrapper -->` prefix; contract enforced by `summary-resolver.test.ts` "wrapper-marker contract" suite. · **FU-8 cleanup (Task §2.5.0b):** drop the unused `response` accumulator on the `send_failed` / `skipped_busy` exit branches in `conversation-initiator.ts:177-190`. Cosmetic; closes M9.4-S4.1 FOLLOW-UPS.md FU-8. |
