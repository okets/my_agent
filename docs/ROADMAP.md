...
| S6.5 | Repairs + Polish | Planned | Design review fixes: wire openTimelineItem(), drop dead delivery field, desktop Home 2x2 grid, unify chat tag injection (generic activeViewContext), dead code sweep (task remnants), referenced automations on space detail, space property view polish (editable name, runtime dropdown, Run button, maintenance rules list), SpaceSyncService use shared readFrontmatter(), timeline polish (one-off badge, spinner, blue dots). [plan](../sprints/m7-s6.5-repairs/plan.md) |
| S7 | E2E Verification | Planned | 14 test tasks: unit tests (handler registry, protection, adapter), integration tests (migration, lifecycle, triggers, HITL), browser tests (calendar, settings UI), live E2E with real LLM calls (handler execution, SDK sessions, HITL resume, debrief MCP), reply routing regression test. [plan](../sprints/m7-s7-e2e-verification/plan.md) |
