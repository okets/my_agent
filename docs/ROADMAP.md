...
| S4.3 | Worker Pipeline Cleanup — Single Writer + Sidecar Metadata (supplemental, planned) | **Planned** | Closes the deferred items from S4.2-fu3's redesign. **(A) Chart augmentation as worker-runtime tool:** today the executor's post-run chart code at `automation-executor.ts:649-672` is the third independent writer of `deliverable.md` — appends `![chart](url)` after worker writes. Post-fu3 it's enrichment-not-corruption (reads worker's clean file), but it still violates the single-writer principle. Move to a worker-callable MCP tool (`chart_tools.append_to_deliverable`) OR teach workers to embed the chart URL inline when writing `deliverable.md` (workers already use `chart_tools.create_chart` to generate the SVG). One write path. **(B) Capability template migration (conditional — trigger when capability workers revive):** `capability_build` and `capability_modify` templates instruct workers to write YAML frontmatter into `deliverable.md`; this conflates user-facing content with framework telemetry. Migrate to `result.json` sidecar — `deliverable.md` is plain markdown for the user, structured metadata (`change_type`, `test_result`, `summary`) lives in `result.json`. Rewrite `completion_report` / `test_executed` / `change_type_set` validators to read the sidecar. Zero active production workers use these templates today (all in `_archive/` post-S4.2-fu1); migration triggers when revived. **(C) Codify the sidecar metadata convention:** add to `packages/dashboard/CLAUDE.md` and the framework docs that structured worker metadata uses `result.json` sidecar (JSON, typed) rather than markdown frontmatter. The project's existing "normalized metadata standard" (markdown frontmatter for markdown files) still applies; sidecar is for structured-data-next-to-markdown which is a different shape. Forward-improvement scope; not blocking morning briefs. Triggered by S4.2 close (clean soak window); only required to close M9.4 supplemental thread. |
