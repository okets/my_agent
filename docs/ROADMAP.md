...
| **M9.6: Capability Resilience & Recovery** | **In Progress** | Phase 1 (S1-S8) + **Phase 2 closed at S15** (S9-S15 done). 5 Phase 3 sprints planned (S16-S20). M10 architecturally unblocked at Phase 2 close; per CTO 2026-04-17, M10 work waits until M9.6 closes at S20. |
...
| S16 | Fix-engine swap + wall-time measurement | **Done** | Orchestrator invokes `capability-brainstorming` skill in fix-mode (Opus, one-shot). ESCALATE markers. `targetPath` plumbing. `JOB_TIMEOUT_MS` = 15 min. Wall-time: 100 s (Branch A, ship as-is). 29/29 tests. [§2.1](sprints/m9.6-capability-resilience/plan-phase3-refinements.md#21-sprint-16--fix-engine-swap-to-capability-brainstorming-fix-mode) |
