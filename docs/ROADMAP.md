...
| S20 | Phase 3 exit gate — two definitive smoke tests | **Planned — unblocked (M9.4-S4.1 merged)** | (1) Working Nina screenshots a website after browser plug deliberately broken. (2) Conversation Nina understands a voice message after STT plug deliberately broken. `AppHarness` + recording mock transport. Abbreviated replays for every other registered plug type. **M9.6 closes here. M10 unblocks.** [§2.5](sprints/m9.6-capability-resilience/plan-phase3-refinements.md#25-sprint-20--phase-3-exit-gate-two-definitive-smoke-tests) · **S20 dev pickup (M9.4-S4.1 FU-2, added 2026-04-20):** CFR-fix automation output contract change — `deliverable.md` becomes a terse one-liner per capability (outcome + file changed, if any; 2–5 lines total across all attempts) because the debrief-reporter aggregator reads `deliverable.md` first. Move forensic detail (diagnosis, decision log, per-attempt state, validation commands) to a sibling file in the same `run_dir` — suggested `attempts.md` or `forensic.md`. Rationale: S4.1 fixed section-drop on the brief side; CFR-fix output still dominates the Haiku 10K condense budget because each attempt produces a ~2–3K forensic deliverable. User-facing brief signal is "voice capability broken, now fixed" — not the three-attempt forensic diary. Full detail stays on disk for audit. Reference: M9.4-S4.1 plan §"Dev note for M9.6-S20" and FOLLOW-UPS.md FU-2. |
