...
| **M9.5: Capability Framework v2** | **Active** | S1-S4 done, S5-S6 planned (test cleanup + screenshot pipeline) |
...
| S5 | Test Cleanup + Deferred Fixes | Planned | Delete 5 orphaned test files (test deleted S3 modules). Fix 2 stale test files (enabled-gate, mock update). Auto-create `.enabled` on first capability build. Structured tool UX feedback from Nina. All tests green — milestone test debt cleared. |
| S6 | Screenshot Pipeline | Planned | Complete VAS storage in screenshot interceptor middleware (spec designed in S1, detection-only implemented in S3). URL injection into tool results for conversation rendering. Desktop screenshots render inline in chat. Browser/Playwright screenshots use same pipeline — generic for any image-producing MCP tool. Verify with Nina: screenshot visible in conversation. |
