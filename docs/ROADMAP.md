...
| S22 | Tool capability recovery loop — close the third shape of the framework | **Planned** | Live retest of `browser-chrome` (2026-04-21) exposed that M9.6's recovery machinery was implicitly STT-shaped: input capabilities work (gate + replay via `reprocessTurn`), output capabilities work in degraded mode (BUG-6 acknowledgment), tool capabilities (browser-control, desktop-control) silently drop the original task after fix because reverify produces no `recoveredContent` and orchestrator's `terminalDrain` skips replay. **BUG-7 (in scope):** add `interaction: input \| output \| tool` frontmatter to CAPABILITY.md (with default-inference for the well-known types) + new `retryTurn` orchestrator path that re-submits the original user turn after a tool capability is fixed. Generic — no per-capability code; works for any future tool capability. **BUG-8 (deferred to M10):** ack arrives after brain has already replied "I can't" — requires SDK pre-delivery hook that doesn't exist today; auto-retry partially papers over it. Live retest gate: CTO breaks `browser-chrome` (real corruption), asks for screenshot, receives it within ~3 min. **M9.6 closes here.** [§2.7](sprints/m9.6-capability-resilience/plan-phase3-refinements.md#27-sprint-22--tool-capability-recovery-loop) · [Gap report](sprints/m9.6-capability-resilience/tool-capability-cfr-gap.md) |
