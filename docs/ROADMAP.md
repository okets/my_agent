...
| S8 | Debrief Worker Architecture | **Planned** | Fix 8 gaps found during real usage: WhatsApp message split, brain mediator framing, worker web access, manifest persistence, debrief worker pipeline (`notify: debrief` collector), debrief reporter system job, conversation voice (no tool narration). Validation: recreate user automations naturally through conversation. [plan](../sprints/m7-s8-debrief-workers/plan.md) |
