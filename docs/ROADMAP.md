...
| **M9.6: Capability Resilience & Recovery** | **In Progress** | Phase 1 (S1-S8) + Phase 2 (S9-S15 done). **Phase 2 closed at S15.** 3 Phase 3 sprints remaining (S16-S18). M10 unblocked. |
...
| S15 | Phase 2 exit gate — E2E incident replay for all installed plug types | Done | TTS CFR detection wired (`synthesizeAudio` → `CapabilityInvoker`). `reverifyTextToAudio` arg contract + MP3 header fix (pre-existing S13 bugs). 4 E2E exit gates: STT real-incident replay (v2 origin, Songkran reprocess), TTS terminal-path (terminal-fixed ack), browser-chrome synthetic (automation-origin, CFR_RECOVERY.md), desktop-x11 synthetic. S7 regression gate passed. **Phase 2 closes here. M10 unblocks.** 290 core tests (2 pre-existing skips). [Plan](sprints/m9.6-capability-resilience/s15-plan.md) · [Review](sprints/m9.6-capability-resilience/s15-review.md) · [Test report](sprints/m9.6-capability-resilience/s15-test-report.md) |
| S16 | Duplicate TTS path collapse | Planned | `chat-service.synthesizeAudio` becomes authoritative (partially done in S15). Per-path fallback table covers error/split/empty cases. Delete Baileys `onSendVoiceReply` synthesis. `reverifyAudioToText` bash-wrapper removal (S10-FU-2 + S13-FU-1). (Was S17 in plan-universal-coverage.md.) |
| S17 | Reflect-phase collapse | Planned | Eliminate REFLECTING state; one job per attempt; budget cap 4 (safety ceiling). Dead-code cleanup. Must come after S15. (Was S13 in plan-universal-coverage.md v2.3; moved to Phase 3 per plan-phase2-coverage.md §0.2.) |
