...
| S4 | Rich I/O | **Done** | Full deliverable pipeline (deliverable.md to disk, not truncated), store_image MCP tool (SVG/base64/URL → PNG via sharp), dashboard image rendering (DOMPurify, lightbox, job detail with full deliverable), WhatsApp outbound images (parse markdown → Baileys media), visual presenter skill, job timeline thumbnails from screenshotIds. 879 tests. [plan](../sprints/m8-s4-rich-io/plan.md) |
