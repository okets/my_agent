...
| S5 | Test Cleanup + Deferred Fixes | Delete 5 orphaned test files (test deleted S3 modules: computer-use-service, desktop-capability-detector, x11-backend, desktop-hooks, desktop-server). Fix 2 stale test files: `capability-system.test.ts` (add `enabled: true` to test capabilities), `session-manager-skills.test.ts` (add `createCapabilityRateLimiter` to mock). Auto-create `.enabled` on first capability build (builder flow or brainstorming skill). Structured tool UX feedback from Nina (specific questions: which tools used, confusing, missing, coordinate scaling, optional tools). | All dashboard tests green. All core tests green. Zero test failures across both packages. Nina feedback logged. |
