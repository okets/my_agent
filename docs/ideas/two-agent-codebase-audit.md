...
3. The `assembleSystemPrompt()` in `packages/core/` assembles the brain's personality prompt. A parallel `assembleWorkingAgentPrompt()` function is needed.
4. Hooks should be wired to both agent types — safety is non-negotiable for working agents too.
