...
- `packages/dashboard/src/chat/chat-service.ts` —
  ...
  - `synthesizeAudio()` (around line 1058) — *do not* refactor in this sprint. S13's reverifier needs both call sites; S17 (Phase 3) collapses the duplicate-TTS path. Adding TTS detection here is straightforward but couples Phase 2 to Phase 3 ordering. Leave a `// TODO(S13/S17): route through invoker` marker.
...
- `packages/core/src/capabilities/reverify.ts` —
  ...
  - Add `runSmokeFixture(failure, registry)` — default for MCP plugs, custom types, anything without an entry in `REVERIFIERS`. Spawns `<capDir>/scripts/smoke.sh` as a **fresh out-of-session subprocess** (not via any in-flight MCP child or SDK session). Treats exit 0 as pass, exit 2 as inconclusive (`{pass: true, recoveredContent: undefined, inconclusive: true}` — per S11 hermeticity rule), exit non-zero/non-2 as fail. If `smoke.sh` is missing, falls back to `cap.status === "available"` with a `console.warn` (template-gap signal).
