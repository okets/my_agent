...
- Zero active capability-build/modify *static manifests* in production. **However** (correction added 2026-05-01 post-fu3-capability-implications audit, `post-fu3-capability-implications.md` §5): `cfr-fix-*` automations are dynamically spawned at runtime by `recovery-orchestrator.ts:412` + `app.ts:758` whenever any registered capability fails. The 26 archived manifests are red herrings; the live spawn path hits the post-fu3 worker pipeline regularly. **Confirmed working post-fu3** — the four frontmatter-aware validators (`completion_report`, `test_executed`, `change_type_set`, `capability_frontmatter`) are still wired and parse YAML correctly; `readAndValidateWorkerDeliverable` strips frontmatter before its body checks; `writePaperTrail` and `recovery-orchestrator.readDeliverable` still read frontmatter fields. The deferred S4.3 sidecar migration is a code-cleanliness improvement, **not** a correctness fix. The original "dormant" framing was wrong.
