...
| 5 | Medium | Stale fact archiving (30-day rule) delegated to Haiku advisory output, not deterministic | Same rationale as #4 — auto-archiving based on date parsing is fragile with varied date formats in knowledge files. Haiku flags stale items in review output for human/agent review. | Stale facts accumulate without manual pruning; acceptable for MVP |
