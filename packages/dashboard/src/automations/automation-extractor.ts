/**
 * Automation Extractor — Task creation + automation matching from conversation
 *
 * Detects task-worthy requests in user messages and extracts structured task data.
 * Also matches messages to active automations via hint keywords.
 * Uses a fast LLM call (Haiku).
 */
...
function buildExtractionPrompt(
  ...
  let basePrompt = `You are a JSON-only task extraction API. You output raw JSON with no other text.
...
or (automation match):
{"shouldCreateTask": false, "matchedAutomation": {"automationId": "<id>", "confidence": 0.0-1.0, "extractedContext": {<structured data from message>}}}`;
...
async function runExtraction(
  ...
  const systemPrompt = buildExtractionPrompt(currentTime, automationHints);
